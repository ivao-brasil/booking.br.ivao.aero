...
export const EventList: FunctionComponent<EventListProps> = ({onEdit}) => {
    ...
    return (
        <div>
            {confirm && (
                <Confirm text={`Please confirm if you want to delete event ${event?.eventName}`}
                         onConfirm={() => event && deleteEvent.mutate(event)} deletion={true}/>
            )}
                    ...
                    events.map(event => (
                        <Grid item xs={4} key={event.id}>
                            <EventCard event={event} onEdit={onEdit} onDelete={onDelete}
                                       slotRedirection={event => navigate(`/admin/events/${event.id}/slots`)}/>
                        </Grid>
                    ))}
            </Grid>
        </div>
